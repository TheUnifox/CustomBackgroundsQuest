...
void OnChangeResolution(float fval)
{
    ...
    if (resmap.count(newval) == 1) {
        element->Text->set_text(il2cpp_utils::newcsstr(resmap[newval]));
    }
    else element->Text->set_text(il2cpp_utils::newcsstr(std::to_string(newval) + " x " + std::to_string(std::round((float)newval * (2.0f / 3.0f)))));
...
void OnChangeRotation(float newval)
{
    ...
    element->CurrentValue = newval;
    element->Text->set_text(il2cpp_utils::newcsstr(std::to_string((int)newval)));
